...
class ProjectWriter
{
    ...
    /**
     * Registrasi dokumen yang byte-nya sudah tersimpan di MinIO (hasil direct
     * multipart upload). Memakai endpoint POST /admin-docs yang sama dengan
    ...
    {
        try {
            $response = $this->externalWrite(timeout: 30)->post($this->apiBase.'/admin-docs', [
                ...$payload,
                'project_id' => $projectId,
            ...
            return $this->result((int) ($body['status'] ?? 0) === 201, $body, $response->status(), fn () => $this->cache->flushDocs($projectId), [
                'name' => 'project',
                'event' => 'created',
                'description' => "Mendaftarkan dokumen project #{$projectId}",
                'properties' => ['project_id' => $projectId, 'key' => $payload['filename']],
            ]);
        } catch (\Throwable $e) {
